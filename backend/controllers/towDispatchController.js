...
const createTowTruckRequest = async (req, res) => {
  try {
    ...
    if (!requestId || !truckId || !driverId || !garageId) {
    ...
    const etaMinutes = Number.parseInt(
      String(estimatedArrivalTime || "").replace(/\D/g, ""),
      10
    );
    ...
    if (!Number.isFinite(etaMinutes) || etaMinutes <= 0) {
      return res.status(400).json({
        success: false,
        message: "A valid estimated arrival time is required.",
      });
    }
    ...
    return res.status(201).json({
      success: true,
      message: "Tow truck request submitted successfully.",
      dispatch: {
    ...
    console.error("Create tow truck request error:", error);
...
const dispatchSelectQuery = `
  SELECT
    ...
    td.service_request_request_id AS requestId,
    td.tow_truck_truck_id AS truckId,
    td.truck_driver_driver_id AS driverId,
    td.assistance_assistance_id AS assistanceId,
    td.dispatch_date AS dispatchDate,
    td.dispatch_time AS dispatchTime,
    td.pickup_location AS pickupLocation,
    td.destination_garage AS destinationGarage,
    td.estimated_arrival_time AS estimatedArrivalTime,
    td.dispatch_status AS dispatchStatus,

    t.truck_number AS truckNumber,
    t.truck_type AS truckType,
    t.truck_model AS truckModel,
    t.truck_status AS truckCategory,
    t.latitude AS truckLatitude,
    t.longitude AS truckLongitude,

    d.full_name AS driverName,
    d.contact_number AS driverContact,
  ...
  LEFT JOIN tow_truck t
    ON t.truck_id = td.tow_truck_truck_id

  LEFT JOIN truck_driver d
    ON d.driver_id = td.truck_driver_driver_id
`;
...
const getPendingTowTruckRequests = async (req, res) => {
  try {
    const [requests] = await db.query(
      `
        ${dispatchSelectQuery}

        WHERE td.dispatch_status = 'Pending Verification'
...
const getTowTruckHistory = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(assistanceId) || assistanceId <= 0) {
      return res.status(400).json({
        success: false,
        message: "A valid assistance officer ID is required.",
      });
    }
    ...
    const [history] = await db.query(
      `
        ...
        WHERE td.assistance_assistance_id = ?
          AND td.dispatch_status <> 'Pending Verification'
    ...
    console.error("Get tow truck history error:", error);
...
const getTowTruckRequestById = async (req, res) => {
  try {
    ...
    if (requests.length === 0) {
      return res.status(404).json({
        success: false,
        message: "Tow truck request was not found.",
      });
    }
    ...
    console.error("Get tow truck request error:", error);
...
const updateTowTruckRequestStatus = async (req, res) => {
  try {
    const dispatchId = Number(req.params.id);
    ...
    if (!allowedStatuses.includes(status)) {
      return res.status(400).json({
        success: false,
        message: "A valid dispatch status is required.",
      });
    }
    ...
    await db.query(
      `
        ...
        WHERE dispatch_id = ?
      `,
      [status, assistanceId, dispatchId]
    );
    ...
    console.error("Update tow truck request status error:", error);
