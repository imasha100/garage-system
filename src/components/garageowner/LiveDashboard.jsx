...
export default function LiveDashboard({ toggleSidebar }) {
  ...
  useEffect(() => {
    ...
    loadDashboardData(garageId, true);

    const interval = setInterval(() => {
      loadDashboardData(garageId, false);
    }, 5000);
  ...
  const globalWorkloadMinutes =
    Number(summary.globalWorkloadMinutes) || 0;

  const activeVehicles =
    Number(summary.activeVehicles) || 0;

  const assignedVehicles =
    Number(summary.assignedVehicles) || 0;

  const totalLiveJobs =
    Number(summary.totalLiveJobs) || 0;

  const totalBays =
    Number(summary.totalBays) || 0;

  const occupancyPercentage =
    Number(summary.occupancyPercentage) || 0;
  ...
  const workloadPercentage =
    Math.min(
      ...
      Math.max(
        0,
        Math.round(
          (globalWorkloadMinutes / 480) * 100
        )
      )
    );

  const workloadLevel =
    globalWorkloadMinutes === 0
      ? "Idle"
      : globalWorkloadMinutes < 120
      ? "Low"
      : globalWorkloadMinutes < 300
      ? "Moderate"
      : "Critical";
  ...
  const handleActionMenu = (jobId) => {
    setOpenActionMenu(
      openActionMenu === jobId
        ? null
        : jobId
    );
  };
