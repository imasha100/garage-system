...
export default function CustomerLogin({
  ...
  const handleContinueRequest = async (
    ...
    const contactNumber =
      continueData.contactNumber
        .trim()
        .replace(/\D/g, "");

    const vehicleNumber =
      continueData.vehicleNumber
        .trim()
        .toUpperCase();
    ...
    try {
      ...
      const restoredRequest = {
        ...
        contact:
          existingRequest.customerContact ||
          contactNumber,

        customerLatitude:
          existingRequest.customerLatitude,

        customerLongitude:
          existingRequest.customerLongitude,
      ...
      if (
        ...
        const restoredGarage = {
          ...
          name:
            existingRequest.garageName ||
            "Selected Garage",

          address:
            existingRequest.garageAddress ||
            "",

          contact:
            existingRequest.garageContact ||
            "",

          lat: Number(
            existingRequest.garageLatitude
          ),

          lng: Number(
            existingRequest.garageLongitude
          ),
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center text-center z-10 my-auto px-4 fade-in">
          ...
          <p className="text-slate-200 text-lg sm:text-xl md:text-3xl tracking-wide max-w-3xl mb-12 font-medium drop-shadow-[0_2px_15px_rgba(0,0,0,0.9)]">
            Create a new roadside
            assistance request or continue
            an existing request.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-stretch gap-4 w-full max-w-3xl">
            ...
            >
              ...
              <span className="block text-sm sm:text-xs font-medium text-slate-400 leading-relaxed">
                ...
                request using your contact
                number and vehicle number.
              </span>
            </button>
          </div>
        </div>
