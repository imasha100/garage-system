...
function OnlineModal({ open, onClose, total, onSuccess }) {
  ...
  const handlePay = () => {
    if (cardNumber.replace(/\s/g, "").length < 16) {
      setError("Give valid card number.");
      return;
    }
    if (expiry.length < 7) {
      setError("Give expiry date (MM / YY).");
      return;
    }
    if (cvv.length < 3) {
      setError("CVV 3 digits .");
      return;
    }
    if (!name.trim()) {
      setError("Give cardholder name.");
      return;
    }
    ...
    <Modal open={open} onClose={onClose}>
      <div className="p-5 space-y-4">
        <div>
          ...
          <h2 className="text-white font-black text-sm mt-0.5">
            Online Payment
          </h2>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {["visa", "master"].map((c) => (
            <button
              ...
              className={`py-2.5 border rounded-sm text-xs font-black tracking-widest ${
                cardType === c
        ...
        <div className="border-t border-slate-900 pt-3 flex items-center justify-between">
          ...
          <button
            onClick={handlePay}
            className="bg-emerald-500 hover:bg-emerald-400 text-black font-black text-xs px-5 py-2.5 rounded-sm"
          >
            PAY NOW
          </button>
        </div>
    ...
    <Modal open={open} onClose={onClose}>
      <div className="p-5 space-y-4">
        ...
        <div className="bg-emerald-900/10 border border-emerald-900 rounded-sm px-3 py-2">
          ...
          <p className="text-emerald-400 font-black text-sm font-mono mt-0.5">
            TXN-9982
          </p>
        ...
        <div className="border-t border-slate-900 pt-3 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="border border-slate-700 hover:border-slate-500 text-slate-300 text-xs px-4 py-2.5 rounded-sm"
          >
            GOT IT
          </button>
        </div>
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="p-6 text-center space-y-4">
        ...
        <button
          onClick={onClose}
          className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-black text-xs py-2.5 rounded-sm"
        >
          DONE
        </button>
      </div>
      ...
      <div className="p-4 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-black py-3 text-xs"
          >
            PAY NOW
          </button>

          <button className="w-full border border-slate-800 hover:border-slate-600 py-2 text-xs flex items-center justify-center gap-2">
